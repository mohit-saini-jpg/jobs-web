...
.sec-body{padding:0}
.kv-table{width:100%;border-collapse:collapse;font-size:.82rem;border:1px solid #111}
.kv-table th{background:#f8fafc;color:#374151;font-weight:700;padding:9px 13px;text-align:left;border:1px solid #111;width:38%;vertical-align:top;word-break:break-word}
.kv-table td{padding:9px 13px;color:#1e293b;font-weight:600;border:1px solid #111;vertical-align:top;word-break:break-word;overflow-wrap:break-word;line-height:1.6}
.kv-table tr:last-child th,.kv-table tr:last-child td{border-bottom:1px solid #111}
...
.data-table{width:100%;border-collapse:collapse;font-size:.81rem;table-layout:auto;border:1px solid #111}
.data-table th{background:#1d4ed8;color:#fff;padding:8px 10px;font-weight:700;text-align:left;white-space:nowrap;border:1px solid #111}
.data-table td{padding:8px 10px;border:1px solid #111;color:#1e293b;font-weight:600;word-break:break-word;vertical-align:top;line-height:1.5}
.data-table tr:nth-child(even) td{background:#f8fafc}
@media(min-width:480px){.data-table{min-width:420px}.data-table th,.data-table td{min-width:60px}}
...
def build_listing_page(title, jobs, canon_url, breadcrumbs, desc='', top_html='', list_noun='Jobs', render_cards=True):
    ...
    # Apply-mode detection (2026-07-26 fix): the old code defaulted straight
    # to 'Online' whenever the structured application_mode/apply_mode field
    # was empty -- wrong for the many Offline/Walk-in postings whose data
    # never got that field cleanly populated. The TITLE is far more reliable
    # here (scrapers consistently write "... Apply Offline", "... Walk-in
    # Interview" etc. into it) so it's checked first; the structured field is
    # only a fallback. If neither says anything, return '' (don't guess) --
    # the listing card then simply omits the apply-mode text instead of
    # showing a mode that might be wrong.
    def _detect_apply_mode(text):
        t = safe(text).lower()
        if re.search(r'walk[\s-]?in', t): return 'Walk-in'
        if 'offline' in t: return 'Offline'
    ...
    for job in jobs:
        ...
        jmode  = _detect_apply_mode(jtitle) or _detect_apply_mode(bd.get('application_mode','') or job.get('apply_mode',''))
        ...
        def _get_link(key):
            ...
            if v and not v.startswith('[') and not v.startswith('{'):
                return v
            return ''
        # NOTE (2026-07-26): deliberately NOT offering a direct external
        # "Apply" quick-link here anymore -- clicking anywhere on the card
        # (including where an Apply button used to sit) should take the user
        # to our own /jobs/{slug}/ content page, not bypass straight to the
        # official portal from the listing page.
...
# Snapshot of pages that existed on disk BEFORE this run's DU processing began.
# _du_slug()'s already-built check below must consult this static snapshot, not
# a live filesystem check — otherwise, when two distinct DU items happen to
# collide on the same generic weak plain-slugify() value (e.g. "2026-pdf" from
# two different Hindi titles that both strip down to just a year + "pdf"),
# whichever one gets processed FIRST in this run writes jobs/2026-pdf/index.html
# for itself, and every OTHER colliding item processed LATER in the SAME run
# then sees that path now exists and wrongly treats it as "already built",
# reusing the wrong (first item's) slug instead of falling back to its own
# transliteration — causing later call sites (e.g. the sections_index-population
# loop) to disagree with the earlier page-writing loop about the correct slug.
_DU_PREBUILT_SLUGS = {p.name for p in (ROOT/'jobs').iterdir() if p.is_dir()} if (ROOT/'jobs').exists() else set()
...
def _du_slug(name, url=''):
    ...
    if has_devanagari and (not s or len(s) < 20):
        already_built = bool(s) and (s in _DU_PREBUILT_SLUGS)
...
# Uses _du_slug() (Devanagari-aware, defined above) rather than plain slugify()
# directly — a Hindi-heavy title strips down to almost nothing in pure ASCII
# (e.g. just "2026-pdf"), and once that weak value gets written back into
# dailyupdates.json here, every later slug computation in this script (and in
# next runs) sees 'slug' already present and just reuses it verbatim, silently
# reintroducing the exact collision/404 bug _du_slug() exists to prevent.
...
# Bug found 2026-07-26 while auditing merge_duplicate_jobs.py: this list was
# missing 'last_date_to_apply' (the actual field name real data uses per
# render_dates()'s PRIO list below) plus several other genuinely-used date
# keys, so two postings with DIFFERENT real deadlines both hashed to an empty
# `dates` string and silently looked identical -- confirmed on a real pair
# (Ministry of Civil Aviation Young Professional: 20-06-2026 vs 30-06-2026)
# that the merge script's dry-run had wrongly flagged as a safe auto-merge.
# Mirrors render_dates()'s PRIO list so "does this signature capture every
# date field real data uses" has one source of truth instead of drifting.
...
def _version_signature(job):
    ...
    if isinstance(d, dict):
        dates = '|'.join(safe(d.get(k)) for k in _VERSION_SIG_DATE_KEYS)
    else:
        dates = safe(d)
    parts = [
        _extract_year(job),
...
for _vst, _vdistricts in VLE_STATE_DISTRICTS.items():
    ...
    for _vd in _vdistricts:
        ...
        # ONE physical page per district (slot 1's URL, no path suffix).
        # A district is NOT capped at one VLE though: vercel.json rewrites
        # any /vle/<state>/<district>/<N>/ (any slot number) to this SAME
        # file, and vle-district.js reads the actual slot straight from
        # the URL at runtime — so VLE #2, #5, #47... never need a new
        # generate_all.py run, just a vle_profiles insert (or a public
...
<script>window.__VLE_STATE__={json.dumps(_vst, ensure_ascii=False)};window.__VLE_DISTRICT__={json.dumps(_vd, ensure_ascii=False)};window.__VLE_SLOT__=1;</script>
