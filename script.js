(() => {
  ...
  // If already has links, don't overwrite
  ...
  // ------------------------------
  // A) Study/Popular/State => jobs.json (STRICT section extraction)
  // ------------------------------
  if (group === "study" || group === "popular" || group === "state") {
    ...
    const arrays = [
      ...(Array.isArray(j?.top_jobs) ? [j.top_jobs] : []),
      ...(Array.isArray(j?.left_jobs) ? [j.left_jobs] : []),
      ...(Array.isArray(j?.right_jobs) ? [j.right_jobs] : [])
    ];

    // Build sections based on {title: "..."} markers
    ...
    arrays.forEach((arr) => {
      arr.forEach((it) => {
        ...
        // Section marker
        if (it.title) {
          current = safe(it.title).toLowerCase();
    ...
    const wantedTitle = {
      study: ["study wise", "study", "qualification", "education", "pass", "class"],
      popular: ["popular", "category", "categories", "department"],
      state: ["state wise", "state", "states"]
    }[group];

    let bestKey = "";
    for (const key of sections.keys()) {
      const k = key || "";
      if (wantedTitle.some((w) => k.includes(w))) {
        bestKey = key;
        break;
      }
    }

    // If we found a matching section, use it
    const sectionItems = bestKey ? (sections.get(bestKey) || []) : [];

    // Fallback: keyword filtering if jobs.json doesn't have titles or match failed
    const stateNames = [
    ...
    const studyKw = ["8th","10th","12th","iti","diploma","graduation","ug","pg","pass","b.a","b.sc","b.com","m.a","m.sc","m.com"];
    const popularKw = ["ssc","railway","rrb","bank","ibps","sbi","police","defence","army","navy","air force","teaching","ctet","neet","upsc","bpsc"];

    const allLinks = [];
    for (const [k, vals] of sections.entries()) {
      (vals || []).forEach((x) => allLinks.push(x));
    }

    let finalItems = sectionItems;

    if (!finalItems.length && allLinks.length) {
      const pickByKw = (x) => {
        const n = safe(x.name).toLowerCase();
        if (group === "study") return studyKw.some((w) => n.includes(w));
        if (group === "popular") return popularKw.some((w) => n.includes(w));
        if (group === "state") return stateNames.some((w) => n.includes(w));
        return false;
      };
      finalItems = allLinks.filter(pickByKw);
    }

    // Last fallback: if jobs.json had no titles at all, use raw pool but still filtered
    if (!finalItems.length && j) {
      const pool = [
        ...(j.left_jobs || []),
        ...(j.right_jobs || []),
        ...(j.top_jobs || [])
      ...
      const rawItems = pool.map((it) => {
        const name = safe(it.name);
        const raw = it.url || it.link || "";
        if (!name || !raw) return null;
        const url = normalizeUrl(raw);
        const external = !!it.external;
        return { name, href: external ? url : openInternal(url, name), external };
      }).filter(Boolean);

      const nLower = (x) => safe(x.name).toLowerCase();
      if (group === "study") finalItems = rawItems.filter((x) => studyKw.some((w) => nLower(x).includes(w)));
      if (group === "popular") finalItems = rawItems.filter((x) => popularKw.some((w) => nLower(x).includes(w)));
      if (group === "state") finalItems = rawItems.filter((x) => stateNames.some((w) => nLower(x).includes(w)));
    }

    if (!finalItems.length) {
      grid.innerHTML = `
        <div class="seo-block">
          <strong>No links found for this category.</strong>
          <p>Your <code>jobs.json</code> may not contain section titles or the names don’t match. Share your jobs.json structure and I’ll map it perfectly.</p>
        </div>
      `;
      return;
    }

    renderButtons(finalItems.slice(0, 180));
    return;
  }

  // ------------------------------
  // B) Admissions / Admit-Result / Khabar / Study Material => dynamic-sections.json
  // ------------------------------
  let d = null;
  try {
    const r = await fetch("dynamic-sections.json", { cache: "no-store" });
  ...
  const sectionTitleByGroup = {
    admissions: ["admission", "counseling", "application", "college", "university"],
    "admit-result": ["admit", "result", "answer key", "syllabus", "exam"],
    khabar: ["khabar", "news", "update", "latest"],
    "study-material": ["study material", "course", "mock", "books", "notes", "pdf"]
  };

  const needles = sectionTitleByGroup[group] || [];
  const sectionsArr = Array.isArray(d?.sections) ? d.sections : [];

  const matched = sectionsArr.filter((s) => {
    const t = safe(s.title).toLowerCase();
    return needles.some((n) => t.includes(n));
  });

  const useSections = matched.length ? matched : sectionsArr;
  ...
  if (!items.length) {
    grid.innerHTML = `
      <div class="seo-block">
        <strong>No links found for this category.</strong>
        <p>Please confirm <code>dynamic-sections.json</code> exists and contains items for this page.</p>
      </div>
    `;
    return;
  }
