(() => {
  ...
  // ---------------------------
  // Homepage search (site-wide) - FIX
  // Searches across: dynamic-sections.json, jobs.json, tools.json, services.json
  // Renders results inside #searchResults (no layout changes).
  // ---------------------------
  function initHomepageSearch() {
    ...
    // Helper: safe innerHTML escaping (we only highlight, not inject raw HTML)
    const escHtml = (s) =>
    ...
    const highlight = (text, q) => {
      ...
      return escHtml(t).replace(re, (m) => `<mark>${m}</mark>`);
    };

    const looksInternal = (u) =>
      /(^[./]|^\/|\.html(\?|#|$))/i.test(safe(u));

    const makeHref = (url, name, external) => {
      const u = safe(url);
      if (!u) return "#";
      if (external === true) return normalizeUrl(u);
      // Internal HTML routes should open directly; external/unknown gets wrapped for safety
      return looksInternal(u) ? normalizeUrl(u) : openInternal(u, name || "");
    };
    ...
    async function buildIndex() {
      ...
      const add = (title, url, source, external) => {
        ...
        const u = safe(url);
        if (!t || !u) return;
        out.push({ title: t, url: u, source: safe(source), external: external === true });
      };

      // 1) Primary internal pages
      [
        ["Home", "index.html"],
        ["Results", "result.html"],
        ["CSC Services", "govt-services.html"],
        ["Tools", "tools.html"],
        ["Helpdesk", "helpdesk.html"],
        ["Study wise jobs", "category.html?group=study"],
        ["Popular job categories", "category.html?group=popular"],
        ["State wise jobs", "category.html?group=state"],
        ["Admissions", "category.html?group=admissions"],
        ["Admit Card / Result / Answer Key / Syllabus", "category.html?group=admit-result"],
        ["Latest Khabar", "category.html?group=khabar"],
        ["Study Material & Top Courses", "category.html?group=study-material"],
      ].forEach(([t, u]) => add(t, u, "Pages", false));

      // 2) dynamic-sections.json
      const dyn = await fetchJson("dynamic-sections.json");
      if (dyn && Array.isArray(dyn.sections)) {
        dyn.sections.forEach((sec) => {
          const secTitle = safe(sec?.title) || "Homepage";
          const items = Array.isArray(sec?.items) ? sec.items : [];
          items.forEach((it) => {
            const name = safe(it?.name);
            const url = safe(it?.url || it?.link);
            if (!name || !url) return;
            add(name, url, secTitle, !!it?.external);
          });
        });
      }

      // 3) jobs.json (supports both {groups:[...]} and {groups:{...}} shapes)
      const jobs = await fetchJson("jobs.json");
      const harvestItems = (arr, label) => {
        if (!Array.isArray(arr)) return;
        arr.forEach((it) => {
          const name = safe(it?.name);
          const url = safe(it?.url || it?.link);
          if (!name || !url) return;
          add(name, url, label, !!it?.external);
        });
      };

      if (jobs) {
        if (Array.isArray(jobs.groups)) {
          jobs.groups.forEach((g) => harvestItems(g?.items, safe(g?.title) || safe(g?.id) || "Jobs"));
        } else if (jobs.groups && typeof jobs.groups === "object") {
          Object.keys(jobs.groups).forEach((k) => harvestItems(jobs.groups[k]?.items, jobs.groups[k]?.title || k));
        }
        // Also support older shapes if present:
        harvestItems(jobs.top_jobs, "Jobs");
        harvestItems(jobs.left_jobs, "Jobs");
        harvestItems(jobs.right_jobs, "Jobs");
      }

      // 4) tools.json (supports {categories:[{title,items}]} or grouped arrays)
      ...
      if (tools) {
        if (Array.isArray(tools.categories)) {
          tools.categories.forEach((cat) => {
            ...
            items.forEach((t) => add(t?.name, t?.url || t?.link, label, !!t?.external));
          });
        } else if (typeof tools === "object") {
          Object.keys(tools).forEach((k) => {
            const arr = tools[k];
            if (!Array.isArray(arr)) return;
            arr.forEach((t) => add(t?.name, t?.url || t?.link, "Tools", !!t?.external));
          });
        }
      }

      // 5) services.json (search service names; open CSC page)
      ...
      svcList.forEach((s) => {
        ...
        if (!name) return;
        add(name, "govt-services.html", "CSC Services", false);
      });

      // Dedupe
      const seen = new Set();
      INDEX = out.filter((x) => {
        const key = (x.title + "||" + x.url + "||" + x.source).toLowerCase();
    ...
    function renderResults(q, rows) {
      ...
      if (!rows.length) {
        host.innerHTML = `
          <div class="mini-links" style="margin-top:10px;">
      ...
      host.innerHTML = `
        <div class="mini-links" style="margin-top:10px;">
          Showing <strong>${top.length}</strong> result(s) for <strong>${escHtml(query)}</strong>
          <button type="button" id="clearHomeSearch" class="btn-primary" style="margin-left:10px;padding:6px 10px;font-size:14px;">Clear</button>
        </div>
        <div class="section-list" style="margin-top:10px;">
          ${top
            .map((r) => {
              const href = makeHref(r.url, r.title, r.external);
              const target = r.external ? ` target="_blank" rel="noopener"` : "";
              const sub = r.source ? `<div class="d">${escHtml(r.source)}</div>` : `<div class="d">Open</div>`;
              return `
                <a class="section-link" href="${href}"${target}>
                  <div class="t">${highlight(r.title, query)}</div>
                  ${sub}
                </a>`;
            })
            .join("")}
    ...
    async function run() {
      ...
      host.innerHTML = `<div class="mini-links" style="margin-top:10px;">Searching…</div>`;
      ...
      const scored = idx
        .map((x) => {
          ...
          let score = 0;
          if (t === qlc) score += 20;
          if (t.includes(qlc)) score += 8;
          if (s.includes(qlc)) score += 2;
    ...
    // Optional: focus search when clicking header icon
    if (openBtn) {
      openBtn.addEventListener("click", () => {
        const block = input.closest(".top-search") || input;
