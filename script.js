(() => {
  ...
  // ---------------------------
  // Site Search (Homepage search bar -> searches whole website)
  // ---------------------------
  function initSiteSearch() {
    // Only wire the main search bar on the homepage
    if (!(page === "index.html" || page === "")) return;

    const main = document.querySelector("main") || document.body;

    // Prefer explicit search inputs, but stay resilient to markup changes
    const input =
      main.querySelector('input[type="search"]') ||
      main.querySelector('input[placeholder*="search" i]') ||
      main.querySelector('#q, #search, #searchInput, #siteSearchInput') ||
      null;
    ...
    function go(q) {
      const query = safe(q);
      if (!query) return;
      // Route to view.html search mode (site-wide search)
      window.location.href = `view.html?q=${encodeURIComponent(query)}`;
    }
    ...
    if (form) {
      ...
      // If there is a nearby button, wire it
      const btn =
        input.parentElement?.querySelector('button[type="submit"], button[data-search-btn]') ||
        main.querySelector('button[type="submit"]');
      if (btn) {
        btn.addEventListener("click", (e) => {
          e.preventDefault();
          go(input.value);
