(() => {
  ...
  // Renders results inside: #searchResults (existing container)
  // Uses input: #siteSearchInput and button: #siteSearchBtn (existing IDs)
  // ---------------------------
  ...
  async function buildSearchIndex() {
    ...
    __searchIndexPromise = (async () => {
      const items = [];
      const pushItem = (name, url, external, source) => {
        const n = safe(name);
        const u = safe(url);
        if (!n || !u) return;
        items.push({
        ...
        if (r.ok) {
          ...
          (data.sections || []).forEach((sec) => {
            (sec.items || []).forEach((it) => {
              pushItem(it.name || it.title, it.url || it.link, !!it.external, sec.title || sec.id || "Homepage");
            });
          });
        }
      } catch (_) {}

      // 2) Jobs dropdown groups (category pages)
      try {
        const r = await fetch("jobs.json", { cache: "no-store" });
        if (r.ok) {
          const data = await r.json();
          (data.groups || []).forEach((g) => {
            // also allow searching the category landing itself
            const gTitle = safe(g.title) || safe(g.id);
            const gId = safe(g.id);
            if (gTitle && gId) pushItem(gTitle, `category.html?group=${encodeURIComponent(gId)}`, false, "Categories");

            (g.items || []).forEach((it) => {
              pushItem(it.name || it.title, it.url || it.link, !!it.external, gTitle || "Categories");
            });
          });
        }
      } catch (_) {}
      ...
      try {
        ...
        if (r.ok) {
          ...
          (data.categories || []).forEach((c) => {
            const cTitle = safe(c.title) || "Tools";
            (c.items || []).forEach((it) => {
              pushItem(it.name || it.title, it.url || it.link, !!it.external, cTitle);
            });
          });
        }
      } catch (_) {}

      // 4) CSC Services (search by name; opens govt-services page for modal)
      try {
        const r = await fetch("services.json", { cache: "no-store" });
        if (r.ok) {
          const data = await r.json();
          ...
          services.forEach((s) => {
            const nm = s.name || s.service || s.title;
            // open CSC page and allow user to click the item there (no new UI)
            pushItem(nm, "govt-services.html", false, "CSC Services");
          });
        }
      } catch (_) {}
      ...
      for (const it of items) {
        const key = (it.name + "|" + it.url).toLowerCase();
  ...
  function renderSearchResults(host, results, query) {
    if (!host) return;
    ...
    if (!top.length) {
      host.innerHTML = `<div class="seo-block"><strong>No results found for “${q}”.</strong><p>Try SSC, Railway, Bank, Police, Result, Admit Card.</p></div>`;
    ...
    host.innerHTML = top.map((r) => {
      ...
      const src = safe(r.source);
      const urlRaw = safe(r.url);
      const isExternal = !!r.external || /^https?:\/\//i.test(urlRaw);

      // Keep behavior consistent with your site:
      // - internal links open directly
      // - external links open in new tab
      const href = isExternal ? normalizeUrl(urlRaw) : normalizeUrl(urlRaw);
      const target = isExternal ? ` target="_blank" rel="noopener"` : "";

      return `
        <a class="section-link" href="${href}"${target}>
          <div class="t">${name}</div>
          <div class="d">${src ? src : (isExternal ? "Open official link" : "Open page")}</div>
        </a>
      `;
    }).join("");
  ...
  async function initTopSearch() {
    // Works anywhere the "top-search" block exists (including homepage)
    ...
    const doSearch = async () => {
      ...
      if (!q) return renderSearchResults(host, [], "");
      const index = await buildSearchIndex();
      const hits = index.filter((it) => {
        const hay = `${it.name} ${it.source} ${it.url}`.toLowerCase();
        return hay.includes(q);
      });
      renderSearchResults(host, hits, input.value);
    };
