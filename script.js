(() => {
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    // installGlobalRedirectGate(); // Disabled: removed redirect popup per user request
  ...
  /** Convert a job title to a clean URL slug (e.g. "Bihar BTSC Instructor 2026" -> "bihar-btsc-instructor-2026") */
  function makeJobSlug(name) {
    return safe(name)
      .replace(/Last\s+Date\s*[:–-]?\s*[\d\/]+/gi, '')  // strip "Last Date: dd/mm/yyyy"
      .replace(/[^a-zA-Z0-9ऀ-ॿ]+/g, '-')       // non-alphanumeric → hyphen
      .replace(/^-+|-+$/g, '')                            // trim leading/trailing hyphens
      .toLowerCase()
      .slice(0, 90);                                      // keep URL readable length
  }

  function buildRedirectUrl(targetUrl, jobName) {
    const to = safe(targetUrl);
    if (!to) return "";
    const hash = urlRedirectFingerprint(to);
    const slug = makeJobSlug(safe(jobName));
    // Put human-readable job slug first so shared URLs are meaningful
    if (slug) {
      return `redirect.html?job=${encodeURIComponent(slug)}&k=${hash}`;
    }
    return `redirect.html?k=${hash}`;
  }
  ...
  function installGlobalRedirectGate() {
    ...
    document.addEventListener("click", (e) => {
      ...
      // Extract job title for clean readable URL (from .t child or anchor text)
      const jobTitle = safe(anchor.querySelector(".t")?.textContent || anchor.textContent).split("\n")[0].trim();
      const target = (anchor.getAttribute("target") || "").trim().toLowerCase();
      const redirectUrl = buildRedirectUrl(normalizedHref, jobTitle);
      if (!redirectUrl) return;
