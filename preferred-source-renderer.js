...
(function () {
  ...
  if (!document.getElementById(CSS_ID)) {
    ...
    .psr-head{display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:linear-gradient(135deg,#7c3aed,#4f46e5);color:#fff;font-weight:800;gap:8px;flex-wrap:nowrap;overflow:hidden;}
    .psr-head-left{display:flex;align-items:center;gap:6px;font-size:.84rem;font-weight:800;white-space:nowrap;min-width:0;flex-shrink:1;}
    .psr-head-left i{flex-shrink:0;}
    ...
    @media(max-width:520px){
      .psr-head{padding:9px 12px;}
      .psr-head-left{font-size:.75rem;}
      .psr-head-right{display:none;}
      .psr-subhead{font-size:.67rem;padding:4px 12px;}
    }
  ...
  function safe(v) {
    ...
    if (typeof v === 'object') {
      ...
      for (const k of ['text','value','name','description','details','qualification','eligibility','content']) {
        if (typeof v[k] === 'string' && v[k].trim()) return v[k].trim();
      }
      // Flatten object to key: value pairs
      return Object.entries(v).filter(([,val])=>val!=null&&val!=='').map(([k,val])=>safe(val)).filter(Boolean).join(' | ');
    }
    return '';
  }
