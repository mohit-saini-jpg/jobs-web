...
(function() {
  'use strict';
  function initFAQ() {
    // Signal to CSS that JS is active → enables accordion (answers collapse).
    document.documentElement.classList.add('js-faq');
    // Collapse all answers except those marked .open (first FAQ stays open).
    document.querySelectorAll('.faq-item').forEach(function(item) {
      var q = item.querySelector('.faq-q');
      var a = item.querySelector('.faq-a');
      if (!q || !a) return;
      if (a.classList.contains('open') || q.classList.contains('open')) {
        q.classList.add('open'); a.classList.add('open'); a.style.display = 'flex';
      } else {
