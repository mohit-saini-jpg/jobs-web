...
async function networkFirstData(request) {
  const url = new URL(request.url);
  // AUDIT FIX: Use stale-while-revalidate instead of no-store
  // Serve cached version instantly, fetch fresh in background
  // This eliminates the "always wait for network" bottleneck
  const swrFiles = [
  ...
  if (swrFiles.some(f => url.pathname.includes(f))) {
    const cache = await caches.open(CACHE_DATA);
    const cached = await cache.match(request);
    // Fetch fresh in background regardless
    const fetchPromise = fetch(request, { cache: 'default' })
