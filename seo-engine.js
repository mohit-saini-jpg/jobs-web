...
(function (w, d) {
  ...
  if (d.readyState === 'loading') {
    d.addEventListener('DOMContentLoaded', function() { init(); initSmartShare(); });
  } else {
    init();
    initSmartShare();
  }
  ...
  /* ══════════════════════════════════════════════════════════════════════
     SMART SHARE — Floating share button for ALL pages
     Job pages: uses window.__TSJ_SHARE_TEXT (set by universal-renderer.js)
     Other pages: auto-generates from document.title + location.href
  ══════════════════════════════════════════════════════════════════════ */
  function addSmartShare() {
    var SHARE_BTN_ID  = 'tsj-smart-share-btn';
    var SHARE_POP_ID  = 'tsj-smart-share-pop';
    var SHARE_STYLE_ID = 'tsj-smart-share-css';

    // Inject CSS once
    if (!d.getElementById(SHARE_STYLE_ID)) {
      var css = d.createElement('style');
      css.id = SHARE_STYLE_ID;
      css.textContent = [
        '#tsj-smart-share-btn{position:fixed;bottom:75px;right:14px;z-index:9990;',
        'width:44px;height:44px;border-radius:50%;background:#f97316;color:#fff;',
        'border:none;cursor:pointer;box-shadow:0 3px 12px rgba(249,115,22,.45);',
        'display:flex;align-items:center;justify-content:center;font-size:1rem;',
        'transition:transform .2s,box-shadow .2s;}',
        '#tsj-smart-share-btn:hover{transform:scale(1.1);box-shadow:0 5px 18px rgba(249,115,22,.6);}',
        '#tsj-smart-share-pop{position:fixed;bottom:128px;right:14px;z-index:9991;',
        'background:#fff;border-radius:14px;box-shadow:0 8px 32px rgba(13,34,87,.18);',
        'border:1px solid #dde3f0;padding:10px 8px;min-width:172px;',
        'display:none;flex-direction:column;gap:6px;',
        'animation:tsj-pop-in .18s ease;}',
        '#tsj-smart-share-pop.open{display:flex;}',
        '@keyframes tsj-pop-in{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}',
        '.tsj-share-opt{display:flex;align-items:center;gap:9px;padding:9px 12px;',
        'border-radius:9px;font-size:.83rem;font-weight:700;cursor:pointer;',
        'border:none;width:100%;text-align:left;transition:background .15s;}',
        '.tsj-share-wa{background:#f0fdf4;color:#16a34a;} .tsj-share-wa:hover{background:#dcfce7;}',
        '.tsj-share-tg{background:#eff6ff;color:#1d4ed8;} .tsj-share-tg:hover{background:#dbeafe;}',
        '.tsj-share-tw{background:#f0f9ff;color:#0369a1;} .tsj-share-tw:hover{background:#e0f2fe;}',
        '.tsj-share-cp{background:#f8fafc;color:#374151;} .tsj-share-cp:hover{background:#f1f5f9;}',
        '.tsj-share-div{height:1px;background:#e2e8f0;margin:2px 4px;}',
        '.tsj-share-label{font-size:.68rem;color:#94a3b8;font-weight:600;',
        'padding:2px 12px 0;text-transform:uppercase;letter-spacing:.04em;}',
      ].join('');
      (d.head || d.body).appendChild(css);
    }

    // Create floating button
    if (!d.getElementById(SHARE_BTN_ID)) {
      var btn = d.createElement('button');
      btn.id = SHARE_BTN_ID;
      btn.setAttribute('aria-label', 'Share this page');
      btn.title = 'Share karo';
      btn.innerHTML = '<i class="fas fa-share-alt"></i>';
      d.body.appendChild(btn);
    }

    // Create popup
    if (!d.getElementById(SHARE_POP_ID)) {
      var pop = d.createElement('div');
      pop.id = SHARE_POP_ID;
      pop.setAttribute('role', 'dialog');
      pop.setAttribute('aria-label', 'Share options');
      pop.innerHTML = [
        '<span class="tsj-share-label">Share karo</span>',
        '<button class="tsj-share-opt tsj-share-wa" data-tsj="wa">',
        '<i class="fab fa-whatsapp"></i> WhatsApp</button>',
        '<button class="tsj-share-opt tsj-share-tg" data-tsj="tg">',
        '<i class="fab fa-telegram-plane"></i> Telegram</button>',
        '<button class="tsj-share-opt tsj-share-tw" data-tsj="tw">',
        '<i class="fab fa-x-twitter"></i> Twitter / X</button>',
        '<div class="tsj-share-div"></div>',
        '<button class="tsj-share-opt tsj-share-cp" data-tsj="cp">',
        '<i class="far fa-copy"></i> Copy Link</button>',
      ].join('');
      d.body.appendChild(pop);
    }

    var shareBtn = d.getElementById(SHARE_BTN_ID);
    var sharePop = d.getElementById(SHARE_POP_ID);

    // Build share text — job pages use __TSJ_SHARE_TEXT, others use title+url
    function getShareText() {
      if (w.__TSJ_SHARE_TEXT) return w.__TSJ_SHARE_TEXT;
      return [
        '🔔 ' + d.title,
        '🌐 ' + w.location.href,
        '📢 India\'s No.1 Sarkari Jobs Portal — topsarkarijobs.com',
    ...
    shareBtn.addEventListener('click', function(e) {
      e.stopPropagation();
      var isOpen = sharePop.classList.contains('open');
      sharePop.classList.toggle('open', !isOpen);
    });

    // Handle share option clicks
    sharePop.addEventListener('click', function(e) {
      var opt = e.target.closest('[data-tsj]');
      if (!opt) return;
      var type = opt.getAttribute('data-tsj');
      var text = getShareText();
      var url  = encodeURIComponent(w.location.href);
      var enc  = encodeURIComponent(text);

      if (type === 'wa') {
        w.open('https://wa.me/?text=' + enc, '_blank', 'noopener');
      } else if (type === 'tg') {
        w.open('https://t.me/share/url?url=' + url + '&text=' + enc, '_blank', 'noopener');
      } else if (type === 'tw') {
        // Twitter: keep under 280 chars — send just first 2 lines + URL
        var tweetLines = text.split('\n').slice(0,3).join('\n');
        if (tweetLines.length > 240) tweetLines = tweetLines.substring(0, 237) + '...';
        w.open('https://twitter.com/intent/tweet?text=' + encodeURIComponent(tweetLines), '_blank', 'noopener');
      } else if (type === 'cp') {
        // Copy full text
        if (navigator.clipboard && navigator.clipboard.writeText) {
          navigator.clipboard.writeText(text).then(function() {
            opt.innerHTML = '<i class="fas fa-check"></i> Copied!';
            opt.style.background = '#dcfce7';
            opt.style.color = '#16a34a';
            setTimeout(function() {
              opt.innerHTML = '<i class="far fa-copy"></i> Copy Link';
          ...
          opt.innerHTML = '<i class="fas fa-check"></i> Copied!';
          setTimeout(function() { opt.innerHTML = '<i class="far fa-copy"></i> Copy Link'; }, 2000);
        }
        return; // Don't close popup on copy
      }
    ...
    d.addEventListener('click', function(e) {
      if (!shareBtn.contains(e.target) && !sharePop.contains(e.target)) {
        sharePop.classList.remove('open');
      }
    });
    ...
    d.addEventListener('keydown', function(e) {
      if (e.key === 'Escape') sharePop.classList.remove('open');
    });
  }

  /* ── Auto-init Smart Share after DOM ready ── */
  function initSmartShare() {
    // Small delay so Font Awesome icons are ready
    setTimeout(addSmartShare, 600);
  }
