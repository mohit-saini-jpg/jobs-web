...
# Same generic "walk every <h2>, capture its tables (structured, un-mixed,
# multi-column matrix-safe) + prose text" extractor already proven for FJA
# content — reused here so Shine (sarkariresultshine.com) pages get the exact
# same deep, complete content_sections output the site renderer already knows
# how to display (render_content_sections_all in generate_all.py).
from fja_parse_helpers import extract_content_sections
...
# Sarkari Result Shine ke pages par baar-baar aane wala promo/notification
# line — isko scraped text/content_sections mein kabhi bhi save nahi karna.
# Curly (“ ”) aur straight (" ") quotes dono variants handle karta hai, aur
# 🔔 emoji optional rakha hai (kabhi missing bhi ho sakta hai).
_PROMO_LINE_RE = re.compile(
    r"\s*🔔?\s*Google\s*पर\s*सबसे\s*पहले\s*और\s*सबसे\s*तेज\s*अपडेट\s*पाने\s*के\s*लिए\s*"
    r"[“\"]\s*Sarkari Result Shine as Your Preferred Source\s*[”\"]\s*पर\s*अभी\s*क्लिक\s*करें\s*&\s*"
    r"इसके\s*बाद\s*[“\"]\s*Take me to Google Search\s*[”\"]\s*पर\s*OK\s*कीजिए\s*\.?",
    re.UNICODE,
)
...
def sr_is_blocked_url(url):
    """Own-site nav / social / app-store link → True (output se hatao).

    Issue-2 fix: job documents (notification/syllabus/result PDFs) — chahe
    sarkariresult.com / doc.sarkariresults.org.in pe host ho — block NAHI hone
    chahiye. Isliye single source of truth `sr_link_is_official` ko delegate
    karte hain (blocked == official nahi). Non-http strings ke liye legacy
    substring fallback (safety)."""
    if not isinstance(url, str) or not url.strip():
        return False
    low = url.strip().lower()
    if not low.startswith("http"):
        return any(b in low for b in SR_BLOCK_URL_PATTERNS)
    return not sr_link_is_official(low)
...
def sr_scrub_text(text):
    ...
    if not isinstance(text, str):
        return text
    # Sarkari Result Shine ka "preferred source" Google-notification promo
    # line — kisi bhi source (Shine ya SR) se aaye, JSON mein kabhi save
    # nahi hona chahiye.
    text = _PROMO_LINE_RE.sub(" ", text)
...
def _shine_lj_extract_important_links(soup, base_url=""):
    ...
    def _lj_is_blocked_url(url):
        ...
        if not url_low.startswith("http"):
            return True
        # Reject malformed URLs whose host has no dot — e.g. "http://not",
        # "http://haryana" (a stray word turned into an href). A real external
        # link always has a proper domain (host with a TLD, i.e. contains a dot).
        try:
            host = urlparse(url_low).hostname or ""
        except Exception:
            return True
        if "." not in host:
            return True
...
def shine_scrape_latest_job(url, listing_title="", listing_date=""):
    ...
    try:
        ...
        # ── Full structured content — every <h2> section's heading + its own
        # tables (structured, isolated, multi-column matrix-safe) + prose text.
        # Same extractor already proven for FJA data (fja_parse_helpers.py) —
        # the site renderer (render_content_sections_all in generate_all.py)
        # already knows how to display this exact shape and already skips
        # sections covered by dedicated renderers (how-to-apply/links/FAQ), so
        # no filtering needed here — capture everything, let the renderer dedupe.
        content_sections = extract_content_sections(container, clean=clean)
        ...
        job = {
            ...
            "minimum_age":      min_age,
            "maximum_age":      max_age,
            # NOTE: age_relaxation_notes / salary_stipend HATA diye — site pe ye
            # page ke bottom pe duplicate/extra sections ban rahe the (main age/
            # salary sections already render hoti hain). faqs/details_page_content
            # bhi neeche hata diye.
            ...
            # faqs HATA diya — site khud auto_generate_faqs se page-specific FAQ
            # banati hai (scraped faqs redundant + neeche duplicate section bante the).
            # details_page_content bhi HATA diya (raw content, "Scholarship Details"
            # jaisa galat extra section ban raha tha).
            "content_sections": content_sections,   # every h2 section: heading+text+tables
        }
...
def shine_scrape_job(url, listing_title="", listing_date=""):
    try:
        ...
        # Full structured content — every <h2> section's heading + its own
        # tables + prose text (same extractor proven for FJA data; the site
        # renderer already knows this exact shape and dedupes at render time).
        content_sections = extract_content_sections(container, clean=clean)
        ...
        job = {
            ...
            "minimum_age":      min_age,
            "maximum_age":      max_age,
            # age_relaxation_notes / age_reference_date / salary_stipend HATA diye
            # (site pe bottom pe duplicate extra sections ban rahe the).
            "selection_process": selection_process,
            ...
            "jobs_info":        shine_extract_jobs_info(soup),
            "vacancy_details":  vacancy_details,
            # faqs / details_page_content HATA diye — site khud FAQ banati hai;
            # details_page_content raw content ka extra section ban raha tha.
            "content_sections": content_sections,   # every h2 section: heading+text+tables
        }
...
def sr_get_homepage_sections():
    ...
    # ── FALLBACK: Direct category-page scrape for 0-item categories ──────────
    # WordPress/GenerateBlocks theme updates change gb-container class IDs.
    # When that happens, Strategy 1 (gb-container ID match) misses sections.
    # Strategies 2-5 try heading/anchor/proximity, but if all fail, directly
    # scrape the category listing page (e.g. sarkariresult.com/latestjob/).
    # This guarantees we ALWAYS get correct data even after site theme changes.
    CATEGORY_DIRECT_URLS = {
        "SR_Latest_Jobs": "https://www.sarkariresult.com/latestjob/",
        "SR_Answer_Key":  "https://www.sarkariresult.com/answerkey/",
        "SR_Result":      "https://www.sarkariresult.com/result/",
        "SR_Admit_Card":  "https://www.sarkariresult.com/admitcard/",
    }
    for cat, direct_url in CATEGORY_DIRECT_URLS.items():
        if result.get(cat):
            continue  # Already populated from homepage — no need to re-scrape
        print(f"  [FALLBACK] {cat} has 0 items from homepage. Trying direct: {direct_url}")
        try:
            fallback_soup = sr_load_page(direct_url)
            if not fallback_soup:
                print(f"  [FALLBACK] Could not load {direct_url}")
                continue
            # Extract all valid links from the direct category page
            anchors = []
            for ul in fallback_soup.find_all("ul", class_=lambda c: c and "wp-block-list" in c):
                anchors.extend(ul.find_all("a", href=True))
            if not anchors:
                for ul in fallback_soup.find_all("ul"):
                    anchors.extend(ul.find_all("a", href=True))
            if not anchors:
                content_div = fallback_soup.find("div", class_="entry-content")
                if content_div:
                    anchors = content_div.find_all("a", href=True)
            if not anchors:
                anchors = fallback_soup.find_all("a", href=True)
            _extract_links_from_anchor_list(anchors, cat, result)
            print(f"  [FALLBACK] {cat}: got {len(result.get(cat, []))} links from direct page")
        except Exception as _fe:
            print(f"  [FALLBACK] {cat} direct scrape error: {_fe}")

    for cat, items in result.items():
        print(f"  [HOMEPAGE-FINAL] {cat}: {len(items)} links")

    # ── RENUMBER SERIALS GLOBALLY per category ────────────────────────────────
    # Multiple gb-containers can map to the same category (e.g. SR_Latest_Jobs
    # has 6 containers). Each container assigned serial from 1 internally →
    # combined list had duplicate/reset serials. Renumber here 1..N so the
    # final JSON has a clean, unique serial for every item in each category.
    for cat, items in result.items():
        for idx, link_obj in enumerate(items, 1):
...
# Heading keyword → SR category (used for dynamic container discovery)
_HEADING_TO_SR_CAT = {
    "latest job":   "SR_Latest_Jobs",
    "latestjob":    "SR_Latest_Jobs",
    "latest":       "SR_Latest_Jobs",
    "answer key":   "SR_Answer_Key",
    "answerkey":    "SR_Answer_Key",
    "answer sheet": "SR_Answer_Key",
    "result":       "SR_Result",
    "admit card":   "SR_Admit_Card",
    "admitcard":    "SR_Admit_Card",
    "admission":    "SR_Admission",
    "syllabus":     "SR_Syllabus",
}

def _classify_by_heading(div):
    """Return SR category key by reading heading text inside a container div."""
    for htag in div.find_all(["h2","h3","h4","h5","h6","b","strong"]):
        txt = htag.get_text(" ", strip=True).lower()
        for kw, cat in _HEADING_TO_SR_CAT.items():
            if kw in txt:
                return cat
    return None


def _extract_anchors_from_div(div):
    """Extract all valid <a> tags from a div using priority patterns."""
    anchors = []
    # Pattern A: wp-block-list
    for ul in div.find_all("ul", class_=lambda c: c and "wp-block-list" in c):
        anchors.extend(ul.find_all("a", href=True))
    # Pattern B: any ul > li > a
    if not anchors:
...
def _strategy_gb_containers(soup, result):
    ...
    DYNAMIC FALLBACK: Agar koi gb-container-* div SR_GB_CONTAINER_MAP mein
    nahi hai (new ID after site update), us div ka heading text padh ke
    category classify karo. Ye ensure karta hai ki WordPress theme update ke
    baad bhi naye containers miss nahi hote.
    """
    if not soup:
        return

    import re
    _gb_pat = re.compile(r"^gb-container-[0-9a-f]{8}$")

    for div in soup.find_all("div", class_=True):
        classes = div.get("class", [])

        # ── Step 1: Try exact SR_GB_CONTAINER_MAP match ──────────────────────
        category_key = None
        for cls in classes:
        ...
        # ── Step 2: Dynamic fallback for unknown gb-container-* IDs ──────────
        # If this div has a gb-container-XXXXXXXX class NOT in the map, try to
        # classify it by heading text. This handles WordPress theme updates that
        # assign new container IDs without requiring a manual map update.
        if not category_key:
            has_gb_id = any(_gb_pat.match(cls) for cls in classes)
            if has_gb_id:
                # Only classify containers that are direct sections (not nested).
                # A direct section usually has a heading tag as a direct/near child.
                category_key = _classify_by_heading(div)
                if category_key:
                    # Learn this new ID so subsequent divs map correctly
                    _new_id = next((c for c in classes if _gb_pat.match(c)), None)
                    if _new_id:
                        SR_GB_CONTAINER_MAP[_new_id] = category_key
                        print(f"  [GB-DYNAMIC] Discovered new container: "
                              f"{_new_id} → {category_key}")
        ...
        anchors = _extract_anchors_from_div(div)
        if anchors:
...
def sr_extract_text_sections(soup):
    ...
    """
    # Section-intent keywords (besides the generic "how to ..." rule below).
    TEXT_SECTION_KEYWORDS = [
        "how to fill", "how to apply", "how to register", "how to download",
        "selection process", "selection procedure",
        "document required", "required documents", "documents required",
        "exam pattern", "exam scheme",
        "important instruction", "general instruction", "important note",
    ]

    # I3 FIX: SR har "How to ..." heading ke aage brand prefix laga deta hai
    # ("Sarkari Result® : How to SBI PO Exam Online Form 2026"). Pehle ye brand
    # prefix ki wajah se poora section reject ho jaata tha. Ab prefix strip
    # karke intent detect karte hain, aur "how to <kuch bhi>" ko accept karte hain.
    BRAND_PREFIX_RE = re.compile(r"^(?:\s*sarkari\s*result[®\s:.\-–—|]*)+", re.I)

    # Video / pure-link label headings ("How to Fill Form (Video Hindi)") — skip
    SKIP_HEADING_RE = re.compile(r"\(?\s*video\s*(?:hindi|english)?\s*\)?", re.I)

    BRAND_RE = re.compile(
        r"(sarkari\s*result|sarkariresult|www\.sarkariresult|"
        r"whatsapp|telegram|join\s+(?:channel|now|group)|"
        r"android\s+app|apple\s+ios|registered\s+trademark|"
        r"support@sarkariresult|disclaimer)",
        re.I
    )

    def _debrand(t):
        return BRAND_PREFIX_RE.sub("", t or "").strip(" :–—-|®")

    def _is_section_heading(text):
        d = _debrand(text).lower()
        if not d:
            return False
        if d.startswith("how to"):
            return True
        return any(kw in d for kw in TEXT_SECTION_KEYWORDS)

    def _clean_line(s):
        """Ek content line se brand/social tokens hatao (poora line drop nahi)."""
        c = clean(SR_BRAND_TEXT_RE.sub("", s or ""))
    ...
    # Candidates: real headings + standalone bold/strong that are NOT already
    # inside a heading (warna ek hi heading do baar match hoke dedup se content
    # waali entry gir jaati thi).
    candidates = list(soup.find_all(["h2", "h3", "h4"]))
    for tg in soup.find_all(["strong", "b"]):
        if not tg.find_parent(["h1", "h2", "h3", "h4"]):
    ...
    for tag in candidates:
        ...
        # Skip video / link-label "how to" headings (no narrative body)
        if SKIP_HEADING_RE.search(low_title):
            continue

        heading_key = low_title.strip()
        if heading_key in seen_headings:
            continue

        # Collect content after this heading (siblings of the heading block)
        content_parts = []
        for sib in tag.next_siblings:
            sib_name = getattr(sib, "name", None)

            if sib_name in ("h2", "h3", "h4"):
                if clean(sib.get_text()):
                    break
            if sib_name == "table":
                break
            if sib_name in ("p", "div"):
                inner = sib.find(["h2", "h3", "h4"])
                if inner and _is_section_heading(clean(inner.get_text())):
                    break

            if sib_name in ("ul", "ol"):
                for li in sib.find_all("li"):
                    lit = _clean_line(li.get_text())
                    if lit and len(lit) > 4 and not BRAND_RE.search(lit):
                        content_parts.append(lit)
            else:
                st = clean(sib.get_text()) if hasattr(sib, "get_text") else str(sib).strip()
                if not st:
                    continue
                cl = _clean_line(st)
                # pure branding/social/disclaimer line → drop
                if not cl or len(cl) < 5 or BRAND_RE.search(cl):
                    continue
                content_parts.append(cl)

            if sum(len(x) for x in content_parts) > 1400:
                break
...
# ── Link classification (Issue-2 fix) ───────────────────────────────────────
# Purani logic har sarkariresult.com / doc.sarkariresults.org.in URL ko block
# kar deti thi — isse notification/syllabus/result PDFs (jo isi domain pe host
# hote hain) bhi gir jaate the. Ab do buckets:
#   HARD BLOCK : social / video / app-store / url-shortener / tools — kabhi job
#                data nahi. In ko capture-time pe hi drop.
#   TEMPLATE   : site ke apne / cross-promo hosts — SIRF tab allow jab URL ek
#                downloadable document ho (PDF/notice); navigation/category
#                pages block.
SR_HARD_BLOCK = [
    "t.me", "telegram.me", "whatsapp.com", "wa.me", "chat.whatsapp",
    "facebook.com", "fb.me", "fb.com", "instagram.com",
    "youtube.com", "youtu.be", "twitter.com", "x.com",
    "linkedin.com", "threads.com", "threads.net", "share.google",
...
SR_TEMPLATE_HOSTS = [
    "sarkariresult.com", "sarkariresultshine.com", "rojgarresult.com",
    "sarkariresultportal.com", "sarkariresults.org.in",
]
# backward-compat alias (kuch purani jagah SR_LINK_BLOCK use karti hain)
SR_LINK_BLOCK = SR_HARD_BLOCK

_SR_DOC_EXT_RE  = re.compile(r"\.(pdf|docx?|xlsx?|pptx?|rtf|zip)(?:[?#]|$)", re.I)
_SR_DOC_PATH_RE = re.compile(
    r"/(?:wp-content/uploads|upload|uploads|files?|documents?|"
    r"attachment|media|download)/", re.I)


def sr_is_document_url(url):
    """URL ek downloadable job document (notification/syllabus/result PDF etc.) hai?"""
    if not url:
        return False
    return bool(_SR_DOC_EXT_RE.search(url) or _SR_DOC_PATH_RE.search(url))


# AWS access key id + S3 presigned-URL credential markers — JSON me kabhi nahi jaane dene
_SR_AWS_KEY_RE = re.compile(
    r'\b(?:AKIA|ASIA|AGPA|AIDA|AROA|AIPA|ANPA|ANVA|ABIA)[0-9A-Z]{16}\b')
_SR_AWS_PRESIGN = ("x-amz-credential", "x-amz-signature", "awsaccesskeyid",
                   "x-amz-security-token")


def sr_strip_url_secret(url):
    """Govt S3 PRESIGNED URLs (jac-projects.s3…?X-Amz-Credential=AKIA…&X-Amz-Signature=…)
    se secret query strip karo — base URL rakho. Warna AWS key JSON me chali jaati
    hai (GitHub push block + credential exposure). Presigned URLs waise bhi expire
    hote hain, isliye credential query drop karna safe hai."""
    if not url:
        return url
    low = url.lower()
    if any(m in low for m in _SR_AWS_PRESIGN) or _SR_AWS_KEY_RE.search(url):
        return url.split("?", 1)[0]
    return url


def sr_resolve_href(href, base_url=None):
    """Relative href (/upload/x.pdf) ko absolute banao. sarkariresult.com default base.
    mailto/tel/js/anchor → '' (skip). AWS presigned-credential query strip."""
    if not href:
        return ""
    href = href.strip()
    if href.startswith(("mailto:", "tel:", "javascript:", "#")):
        return ""
    if href.startswith("//"):
        return sr_strip_url_secret("https:" + href)
    if href.startswith("http"):
        return sr_strip_url_secret(href)
    base = base_url or "https://www.sarkariresult.com"
    try:
        return sr_strip_url_secret(urljoin(base, href))
    except Exception:
        return ""


def _sr_url_host(url):
    """URL ka hostname (lowercase). Parse fail → ''."""
    try:
...
def _sr_host_in(host, domains):
    """host domain-list me hai? Exact ya subdomain match (substring NAHI —
    warna 'result.com' me 't.co' jaisa false-positive ho jaata hai)."""
    for d in domains:
...
def sr_link_is_official(url):
    ...
    if not low.startswith("http"):
        return False
    host = _sr_url_host(low)
    if not host:
        return False
    # HARD BLOCK — social/video/app/shortener/tools (host-based, exact/subdomain)
    if _sr_host_in(host, SR_HARD_BLOCK):
        return False
    # Job documents hamesha rakho — chahe site ke apne domain pe hi ho
    # (sarkariresult.com/wp-content/uploads/…Advt.pdf, doc.sarkariresults.org.in/…Syllabus.pdf)
    if sr_is_document_url(low):
        return True
    # Site ke apne / cross-promo navigation & category pages → job data nahi
    if _sr_host_in(host, SR_TEMPLATE_HOSTS):
        return False
    return True
...
def sr_build_important_links(useful_links_arr):
    ...
    Har URL se AWS presigned-credential query strip (central chokepoint — kisi bhi
    link-path se aaya presigned URL yahan clean ho jaata hai)."""
    out, seen = [], set()
    for it in useful_links_arr or []:
        label = sr_scrub_text(it.get("title", "")).strip()
        url   = sr_strip_url_secret((it.get("url", "") or "").strip())
...
def sr_collect_all_links(soup, content_scope=None, base_url=None):
    """
    ...
    Relative hrefs (/upload/x.pdf) ko absolute resolve karta hai. Returns [{title,url}].
    """
    scope = content_scope or soup.find("div", class_="entry-content") or \
            soup.find("article") or soup
    out, seen = [], set()
    for a in scope.find_all("a", href=True):
        href = sr_resolve_href(a.get("href", ""), base_url)
...
def sr_deep_extract(soup):
    ...
    _CATWISE_COLS = re.compile(
        r"^(gen|general|ur|obc|sc|st|ews|ph|pwd|pwbd|dviyang|divyang|exsm|esm|"
        r"unreserved|total|female|male|others?|"
        r"bc-?a|bc-?b|bc-?c|bc|sbc|mbc|dff|dsp|dv|tf|ph-?[hbo]?)$", re.I
    )
        ...
        for tr in rows:
            ...
            if mode == "catwise":
                ...
                if len(texts) >= 2 and catwise_headers:
                    ...
                    # FIX (HPSC-type): agar catwise_headers[0] KHUD ek category hai
                    # (Gen/General/UR/EWS/OBC/SC/ST/Total) to is table me koi Post-Name
                    # column hai hi nahi — poori row category counts hain. Warna
                    # headers[0]="Gen" ka value (902) galti se postName ban jaata tha
                    # aur Gen category kho jaati thi. Aise me post_name blank rakho
                    # (neeche previous real post pe fallback) aur SAARE headers→values map.
                    _first_is_cat = bool(catwise_headers and
                                         _CATWISE_COLS.match(catwise_headers[0].strip()))
                    post_name = ""
                    aligned_values = []
                    if _first_is_cat:
                        # no post-name column — map all headers to all values
                        post_name = ""
                        _cw_hdrs = list(catwise_headers)
                        aligned_values = list(raw_cells)
                    elif len(raw_cells) >= len(catwise_headers):
                        post_name = sr_scrub_text(raw_cells[0]) if raw_cells else ""
                        aligned_values = raw_cells[1:]
                        _cw_hdrs = catwise_headers[1:]
                    else:
                    ...
                    cw = {}
                    _cw_total = ""
                    for i, hdr in enumerate(_cw_hdrs):
    ...
    # I3 FIX: pehle "How to Fill" video-label heading pe h.find_next("ul") poore
    # document me kahin bhi jaake ek unrelated <ul> (FAQ-type sentence) utha leta
    # tha → fabricated howToApply. Ab (1) brand-prefix strip karke "how to ..."
    # detect karo, (2) video/link-label headings skip, (3) sirf usi heading ke
    # aas-paas ki (same-parent next-sibling) <ul> lo — cross-section jump nahi.
    _HTA_BRAND_PREFIX = re.compile(r"^(?:\s*sarkari\s*result[®\s:.\-–—|]*)+", re.I)
    for h in scope.find_all(["h2", "h3", "h4"]):
        _raw = h.get_text(" ", strip=True)
        _debr = _HTA_BRAND_PREFIX.sub("", _raw).strip(" :–—-|®").lower()
        htext = h.get_text(" ").lower()
        _is_how = (_debr.startswith("how to") or
                   re.search(r"apply.*online form|filling", htext))
        _is_video = bool(re.search(r"\(?\s*video\s*(?:hindi|english)?\s*\)?", _debr))
        if _is_how and not _is_video:
            # nearest <ul> that is a real sibling of the heading (same container)
            ul = None
...
# ── LOCATION-WISE VACANCY TABLES (district / state / zone / division wise) ────
# SR posts (Anganwadi / Rojgar Sevak / Shiksha Mitra type) cram a
# "District Wise Vacancy Details" sub-table INSIDE the mega <table>: a full-width
# <h2> heading row, then a column-header row (District Name | Total Vacancy |
# Last Date | <notification/village-ward link>), then per-district data rows.
# The generic mega-table salvage LUMPS these rows under the WRONG heading
# (the preceding eligibility "Vacancy Details") and mixes them with other rows,
# so the site renders a broken / mis-named table. This dedicated parser pulls
# each location-wise sub-table out CLEANLY — its own real heading + header row +
# all data rows with every column preserved (District, Total, Last Date, link).
# first column label of a location-wise header row (District Name / State / Zone…)
_LOC_COL0_RE = re.compile(
    r"^(district|state|zone|division|region|circle|city|village|ward|block|"
    r"tehsil|taluka|mandal|panchayat)\b", re.I)
# other columns that confirm it's a vacancy/location breakdown (not some other grid)
_LOC_COLX_RE = re.compile(
    r"total|vacanc|post|last\s*date|notification|download|detail|form", re.I)
_LOC_STOP_RE = re.compile(
    r"how to (fill|apply)|some useful|important link|useful link|"
    r"interested candidate|frequently asked|\bfaq\b|comments?\s*(are\s*)?closed|"
    r"download the sarkari|join sarkari|application fee|important date|"
    r"age limit|selection process|eligibility detail", re.I)


def sr_extract_location_wise_tables(soup):
    """Clean extractor for district/state/zone/division-wise vacancy tables.
    HEADER-DRIVEN: finds the column-header row whose first cell is a location
    label (District Name / State / Zone …) + at least one Total/Last-Date/
    Notification column, then attaches the nearest preceding <h2> heading and
    collects all following data rows. This is robust even when SR mislabels the
    section heading (e.g. writes 'Category Wise Vacancy Details' above a table
    whose columns are actually District Name | Total | Last Date | Notification).
    Returns [{heading, headers, rows}] — colspan-aware (merged cells collapse to
    one value so header and data rows line up)."""
    scope = (soup.find("div", class_="entry-content") or soup.find("article") or soup)
    tables_out = []
    for table in scope.find_all("table"):
        rows = table.find_all("tr")
        ...
        i = 0
        while i < n:
            cells = [sr_scrub_text(c.get_text(" ")) for c in rows[i].find_all(["td", "th"])]
            nonempty = [c for c in cells if c]
            joined = " ".join(nonempty)
            # track single-cell section headings (candidate heading for a table below)
            if len(nonempty) == 1 and len(joined) < 140:
                last_heading = re.sub(
                    r'(sarkari\s*result|www\.sarkariresult|since\s*201\d)[^\n]*',
                    '', joined, flags=re.I).strip()
                i += 1
                continue
            # location-wise column-header row?
            is_loc_header = (
                len(nonempty) >= 2
                and _LOC_COL0_RE.match(nonempty[0])
                and any(_LOC_COLX_RE.search(c) for c in nonempty[1:])
            )
            if not is_loc_header:
                i += 1
                continue
            hdr_cells = cells
            heading = last_heading
            # collect data rows until next heading / section stop
            data = []
            any_href = False
            k = i + 1
            while k < n:
                tds = rows[k].find_all(["td", "th"])
                rc = [sr_scrub_text(c.get_text(" ")) for c in tds]
                rne = [c for c in rc if c]
                rj = " ".join(rne)
                if not rne:
                    k += 1
                    continue
                # another location header / section heading → stop
                if len(rne) == 1 and len(rj) < 140:
                    break
                if _LOC_STOP_RE.search(rj) or len(rj) > 300:
                    break
                # capture the notification/PDF/village-wise link href — usually
                # sits in the last cell (e.g. "Moradabad Anganwadi Worker Form
                # 2026"). Without this the URL was silently dropped and only
                # the link TEXT survived into the JSON.
                href = ""
            ...
            if data:
                if any_href:
                    # keep every row the same width — append href ("" if a
                    # particular row had none) and add a matching header col
                    out_rows = [rc + [href] for rc, href in data]
                    out_headers = hdr_cells + ["PDF Link"]
                else:
                    out_rows = [rc for rc, _ in data]
                    out_headers = hdr_cells
                tables_out.append({"heading": heading,
                                   "headers": out_headers,
                                   "rows": out_rows})
            i = k if k > i else i + 1
    return tables_out
...
def sr_scrape_detail(category, url):
    ...
    page_links = sr_collect_all_links(soup, base_url=url)
    ...
    _ROW_JUNK = re.compile(
        ...
        r"for\s*the\s*latest\s*updates|"
        # SR template tool/app promo rows (Image Resizer, Resume CV Maker, Typing
        # Test | Sarkari Result Tools · Android Apps | Apple IOS Apps) — junk
        r"image\s*resizer|resume\s*cv|typing\s*test|jpg\s*to\s*pdf|"
        r"android\s*apps?|apple\s*ios|mobile\s*apps?|"
        r"^\s*apps?\s*$|^\s*tools?\s*$|^\s*portal\s*$|"
        r"practice\s*and\s*more", re.I)
    # I4 FIX: headings whose data is FULLY structured elsewhere → skip whole table.
    # NOTE: "vacancy detail"/"category wise"/"district wise"/"post wise" ko yahan se
    # HATA diya — SR in mega-tables me vacancy ke saath district/location-wise
    # breakdown (District | Total Vacancy | Last Date) bhi cram karta hai jo
    # vacancyDetails me capture nahi hoti. Ab ye tables wholesale-skip nahi hongi;
    # niche row-level salvage inme se already-captured (post-name/eligibility) rows
    # drop karke baaki breakdown rows dataTables me rakhega. Fully-captured vacancy
    # tables `vac_post_set` overlap check se already skip ho jaati hain.
    _STRUCT_TBL = re.compile(
        r"subject.?wise|course detail|important link|useful link|"
        r"application fee|important date|"
        r"eligibility detail|physical eligib|age limit|how to (fill|apply)|"
        r"selection process|pay scale|salary|short information", re.I)
    ...
    _TOOL_CELL = {"apps", "app", "tools", "tool", "portal", "website"}
    def _row_is_junk(row):
        blob = " ".join(str(c) for c in row).strip()
        if not blob:
            return True
        # SR template tool/app-promo row: koi bhi cell sirf "Apps"/"Tools"/"Portal"
        # ho (e.g. ["s","Apps"], ["Typing Test Practice and More","Tools"]) → junk.
        _cells_low = [str(c).strip().lower() for c in row if str(c).strip()]
        if any(c in _TOOL_CELL for c in _cells_low):
            return True
        # I4 FIX: row jiska pehla cell ek post name hai jo vacancyDetails me
        # already capture ho chuka → duplicate, drop. (District/location breakdown
        # rows ka pehla cell district name hota hai — vac_post_set me nahi — so
        # wo survive karti hain.)
        if row:
            _c0 = str(row[0]).strip().lower()
            if _c0 and _c0 in vac_post_set:
                return True
    ...
    data_tables = []
    # category-wise breakdown ab vacancyDetails.categoryWise me captured hai —
    # us table ko dataTables me DUPLICATE mat karo (warna galat heading + double data).
    _has_catwise = any(isinstance(v, dict) and v.get("categoryWise")
                       for v in vacancy_out)
    _CAT_CODE = re.compile(
        r"^(gen|general|ur|obc|sc|st|ews|ph|pwd|pwbd|divyang|dviyang|exsm|esm|"
        r"unreserved|total|total\s*post|female|male|others?|"
        r"bc-?a|bc-?b|bc-?c|bc|sbc|mbc|dff)$", re.I)

    def _is_category_table(rows):
        for r in rows:
            cells = [str(c).strip() for c in r if str(c).strip()]
            if len(cells) >= 3 and all(_CAT_CODE.match(c) for c in cells):
                return True
        return False

    # ── LOCATION-WISE tables (district/state/zone/division) — CLEAN dedicated
    #    capture. Each renders as its OWN table with its OWN real heading. The
    #    mega-table salvage lumps these under the wrong heading, so we (a) capture
    #    them cleanly here, (b) suppress the messy salvage duplicate below, and
    #    (c) strip any location rows that leaked into vacancyDetails.
    _loc_tables = sr_extract_location_wise_tables(soup)
    _loc_first = set()
    for _lt in _loc_tables:
        for _r in _lt.get("rows", []):
            if _r and str(_r[0]).strip():
                _loc_first.add(str(_r[0]).strip().lower())
    ...
    for t in deep.get("allTables", []):
        ...
        if heading and _STRUCT_TBL.search(heading):
            continue
        # category-wise breakdown table (Gen|EWS|BC-A|SC|Total) already in categoryWise
        if _has_catwise and _is_category_table(rows):
            continue
        # location-wise breakdown already captured cleanly (dedicated table) →
        # skip the messy mega-table salvage duplicate (same district/state rows)
        if _loc_first:
            _lc = sum(1 for r in rows if r and str(r[0]).strip().lower() in _loc_first)
            if _lc >= 2:
                continue
    ...
    # append the CLEAN location-wise tables (own real heading + all columns).
    # These render as separate, correctly-named tables (no broken/mixed rows).
    for _lt in _loc_tables:
        _lt_rows = [r for r in _lt.get("rows", []) if not _row_is_junk(r)]
        if not _lt_rows:
            continue
        data_tables.append({"heading": _lt.get("heading", ""),
                            "headers": _lt.get("headers", []),
                            "rows": _lt_rows})
    # drop any location rows that leaked into vacancyDetails (now in a clean table)
    if _loc_first:
        vacancy_out = [v for v in vacancy_out
                       if not (isinstance(v, dict) and
                               (v.get("postName") or "").strip().lower() in _loc_first)]
...
def sn_scrape_listing(page_url, category, limit, existing_items=None):
    ...
    _existing_by_url = {it["_scraped_from"]: it for it in (existing_items or [])
                        if it.get("_scraped_from")}
    # RE-SCRAPE-ON-UPDATE: carry-forward an already-scraped job only if it isn't
    # due for a refresh; an ACTIVE job is re-fetched so source updates (extended
    # dates / corrigendum) get captured. Expired jobs are never re-fetched.
    from fja_parse_helpers import needs_refresh as _needs_refresh
    _carry_forward = [_existing_by_url[l] for l in links
                      if l in _existing_urls and not _needs_refresh(_existing_by_url[l])]
    new_links = [l for l in links
                 if l not in _existing_urls or _needs_refresh(_existing_by_url.get(l))]
    print(f"  [INCREMENTAL] {category}: {len(_carry_forward)} carried "
          f"(skipped), {len(new_links)} to fetch (new + active re-check)")
    links = new_links
    ...
    for idx, link in enumerate(links, 1):
        ...
        try:
            ...
            seen_titles.add(key)
            item["sequence"] = idx
            from fja_parse_helpers import now_iso as _now_iso
            item["_last_checked"] = _now_iso()   # INTERNAL — drives needs_refresh()
            results.append(item)
        except Exception as e:
...
def sarkari_main():
    ...
    _shine_existing_items = {j["_scraped_from"]: j for j in _existing_by_cat.get("OFFLINE_FORM", [])
                             if j.get("_scraped_from")}
    # ORDER-PRESERVING INCREMENTAL: carry-forward items ko page order mein rakhna zaroori hai.
    # Pehle code mein carry-forward items PEHLE aa jaate the, phir new items — is wajah se
    # website ka original page order toot jaata tha. Ab FULL ordered list (top-30) maintain
    # karke har position pe correct item rakhte hain.

    # Build lookup of carry-forward items by URL
    _shine_carry_by_url = {
        obj["href"]: _shine_existing_items[obj["href"]]
        for obj in shine_link_objs_top30
        if obj["href"] in _offline_existing_urls
    }
    _shine_new_objs = [obj for obj in shine_link_objs_top30
                       if obj["href"] not in _offline_existing_urls]
    print(f"  [INCREMENTAL] Offline Form: {len(_shine_carry_by_url)} already scraped "
          f"(skipped), {len(_shine_new_objs)} new to fetch")
    print(f"PROCESSING {len(_shine_new_objs)} NEW LINKS:")
    for i, obj in enumerate(_shine_new_objs, 1):
        print(f"  [{i:02d}] {obj['date']}  |  {obj['title'][:70]}")

    # Step 3: Scrape new detail pages IN ORDER using threads, then re-sort by original index
    shine_results_by_href = {}
    with ThreadPoolExecutor(max_workers=SHINE_MAX_THREADS) as executor:
        future_to_idx = {
            ...
            for idx, obj in enumerate(_shine_new_objs)
        }
        for future in as_completed(future_to_idx):
            ...
            if result:
                ...
                shine_results_by_href[_shine_new_objs[idx]["href"]] = result

    # Step 4: Re-assemble in ORIGINAL PAGE ORDER using the full top-30 list.
    # Each position: carry-forward item OR freshly scraped item, in website order.
    shine_jobs = []
    for obj in shine_link_objs_top30:
        href = obj["href"]
        if href in _shine_carry_by_url:
            shine_jobs.append(_shine_carry_by_url[href])
        elif href in shine_results_by_href:
            shine_jobs.append(shine_results_by_href[href])
        # else: scrape failed — skip this position
    ...
    _lj_existing_items = {j["_scraped_from"]: j for j in _existing_by_cat.get("LATEST_JOBS NEW", [])
                          if j.get("_scraped_from")}
    # ORDER-PRESERVING INCREMENTAL: same fix as offline form — carry-forward items
    # must NOT be pre-pended; they must sit at their original page positions.
    _lj_carry_by_url = {
        obj["href"]: _lj_existing_items[obj["href"]]
        for obj in shine_lj_top100
        if obj["href"] in _latestnew_existing_urls
    }
    _lj_new_objs = [obj for obj in shine_lj_top100
                    if obj["href"] not in _latestnew_existing_urls]
    print(f"  [INCREMENTAL] Latest Jobs New: {len(_lj_carry_by_url)} already scraped "
          f"(skipped), {len(_lj_new_objs)} new to fetch")
    print(f"PROCESSING {len(_lj_new_objs)} NEW LINKS:")
    for i, obj in enumerate(_lj_new_objs, 1):
        print(f"  [{i:03d}] {obj['date']}  |  {obj['title'][:70]}")

    # Step 3: Scrape new detail pages via threads, preserve index
    shine_lj_results_by_href = {}
    with ThreadPoolExecutor(max_workers=SHINE_MAX_THREADS) as executor:
        future_to_idx = {
            executor.submit(shine_scrape_latest_job, obj["href"], obj["title"], obj["date"]): idx
            for idx, obj in enumerate(_lj_new_objs)
        }
        for future in as_completed(future_to_idx):
            idx = future_to_idx[future]
            result = future.result()
            if result:
                result["sequence"] = idx + 1
                shine_lj_results_by_href[_lj_new_objs[idx]["href"]] = result

    # Step 4: Re-assemble in ORIGINAL PAGE ORDER using the full top-100 list.
    shine_lj_jobs_ordered = []
    for obj in shine_lj_top100:
        href = obj["href"]
        if href in _lj_carry_by_url:
            shine_lj_jobs_ordered.append(_lj_carry_by_url[href])
        elif href in shine_lj_results_by_href:
            shine_lj_jobs_ordered.append(shine_lj_results_by_href[href])
        # else: scrape failed — skip
    ...
    for category, links in homepage_sections.items():
        ...
        # ACTIVE_FILTER REMOVED — it silently skipped result/mains posts
        # (e.g. UPPSC LT Grade Result page) that had an old lastDateApplyOnline
        # but are still live and relevant. SR_Latest_Jobs can contain results,
        # admit cards, and new forms — we must NOT filter by apply deadline.
        ACTIVE_FILTER = set()   # empty = no active-date filtering for any category
        ...
        _sr_by_src = {(it.get("meta") or {}).get("sourceUrl", ""): it for it in _existing_items
                      if (it.get("meta") or {}).get("sourceUrl")}
        # RE-SCRAPE-ON-UPDATE: also re-fetch already-scraped ACTIVE jobs due for a
        # refresh (needs_refresh), so source changes get captured; expired skip.
        from fja_parse_helpers import needs_refresh as _needs_refresh
        _new_link_objs = [l for l in links
                          if l.get("url") not in _existing_url_set
                          or _needs_refresh(_sr_by_src.get(l.get("url")))]
        print(_ic.summary_line(category, len(links), len(_new_link_objs), len(_existing_url_set)))

        # ── ORDER-PRESERVING SR INCREMENTAL SCRAPE ─────────────────────────────
        # ROOT CAUSE FIX: carry-forward items pehle arbitrary order mein _existing_items
        # se aate the (pichle JSON ka order), phir new items append hote the —
        # is wajah se combined list homepage listing order follow nahi karti thi.
        # Fix: har link ke liye (puri today's list mein) ek ordered_link_results
        # dict banao, phir end mein homepage order se rebuild karo.

        _new_urls = {l.get("url") for l in _new_link_objs}
        _today_urls = {l.get("url") for l in links}        # links = FULL today's list (sliced)
        _today_url_ordered = [l.get("url") for l in links]  # preserves homepage order

        # Build a lookup of existing items by their sourceUrl
        _existing_by_source_url = {}
        for _it in _existing_items:
            _su = (_it.get("meta") or {}).get("sourceUrl", "")
            if _su:
                _existing_by_source_url[_su] = _it

        # Scrape only the NEW links (same logic as before)
        _newly_scraped_by_url = {}
        links_new = _new_link_objs   # only the NEW links go through the fetch loop below
        for item in links_new:
            try:
                ...
                detail = sr_stamp_category(detail, category,
                                           ...
                                           item.get("url", ""))
                from fja_parse_helpers import now_iso as _now_iso
                detail["_last_checked"] = _now_iso()   # INTERNAL — needs_refresh()
                _newly_scraped_by_url[item["url"]] = detail
            except Exception as e:
                print("    SR ERROR:", e)
                continue

        # Re-assemble category_items in HOMEPAGE LISTING ORDER (today's page order)
        # — carry-forward items OR freshly scraped items, whichever applies for each URL.
        category_items = []
        _seen_keys_ordered = set()
        for _url in _today_url_ordered:
            if _url in _newly_scraped_by_url:
                it = _newly_scraped_by_url[_url]
            elif _url in _existing_by_source_url:
                # carry the existing item — covers both not-refreshed jobs AND a
                # refresh whose re-fetch failed (never lose already-good data).
                it = _existing_by_source_url[_url]
            else:
                continue
            _title_key = (it.get("title") or "").lower()
            if _title_key and _title_key in _seen_keys_ordered:
                continue
            if _title_key:
                _seen_keys_ordered.add(_title_key)
